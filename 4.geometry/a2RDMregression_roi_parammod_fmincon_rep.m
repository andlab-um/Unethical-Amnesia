base_dir='/home/haiyanwu/nas_data/experiment_data/uarep_only/5.geometry/';
...
ROIs={'BA9-dlPFC','BA24-dACC','BA34-hip','lDLPFC','rDLPFC','ventral_striatum','putamen','rTPJ','lTPJ','BA21-MTG','BA23-PCC','BA47-vlPFC','mPFC','NAcc','ACC','BA33-rACC','amyg','amygL','amygR','precuneus','caudate','BA45-IFG','BA10-FPC'};
...
for r=1:length(ROIs)
   ...
   for i=1:length(subList)
        ...
        brainRDMs = xlsread([base_dir 'brainRDMs_rep.xlsx'],roiname);
        brainRDM=brainRDMs(:,i+1);
        brainRDM=brainRDM(isnan(brainRDM)==0,:);
        
        
        
        comb=xlsread([base_dir 'combination_rep.xlsx'],num2str(subno));
        con_raw= comb(2:end,2);
        mon_raw= comb(2:end,3);
        islie_raw= comb(2:end,4);
        
        %con_raw=normalize(con_raw0);
        %mon_raw=normalize(mon_raw0);
        %islie_raw=islie_raw0;
...
function rsa_roi_parammod_fmincon(subno,roiname,brainRDM,con_raw,mon_raw,islie_raw,base_dir)
  ...
  parfor it = 1:N_ITERS
        ...
        loss = @(initvals)sum((y_sub - ParamModelRDM(initvals,con_raw,mon_raw,islie_raw)).^2);
    ...
    parsave([base_dir 'results/results_fmincon_' num2str(N_ITERS) 'iter_parametrised_rdms_' roiname '_sub-' num2str(subno)], results);
